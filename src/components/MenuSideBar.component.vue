		...
		<section>
			...
			<nav>
				<q-tabs v-model="tab" vertical inline-label indicator-color="primary" active-color="white" content-class="menu-side-bar">
					...
					<q-route-tab name="Calendar" label="Calendar" to="/" exact icon="sym_r_calendar_month" @click="notifyToast_NotImplemented()" />
					<q-route-tab name="Tasks" label="Tasks" to="/" exact icon="sym_r_checklist" @click="notifyToast_NotImplemented()" />
					<q-route-tab name="Notes" label="Notes" to="/" exact icon="sym_r_note_stack" @click="notifyToast_NotImplemented()" />
				</q-tabs>
			</nav>
			...
			<div class="upgrade-to-pro-wrapper">
				...
				<div>
					<p>to get access too all features!</p>
					<p>Connect with ProTrack World!</p>
				</div>
			</div>
		</section>
		<section>
...
<style scoped lang="scss">
	.menu-side-bar-wrapper {
		...
		.upgrade-to-pro-wrapper {
			...
			color: $background;
			background: $tertiary;
			background: linear-gradient(180deg, rgba($tertiary, 1) 0%, rgba($tertiary, 0.33) 100%);

			.decorative-logo {
				position: absolute;
				top: calc(0px - 100% / 2.8 / (2.8 / 2));
				display: flex;
				justify-content: center;
				width: calc(100% / 2.8);
				border-radius: 50%;
				...
				overflow: hidden;
				box-shadow: 0px 24px 73px 0px rgba(0, 0, 0, 0.1);
